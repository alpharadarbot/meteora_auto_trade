...
class DLMM:
    ...
    def initialize_position_and_add_liquidity_by_strategy(
        self, 
        position_pub_key: Pubkey, 
        user: Pubkey, 
        x_amount: int, 
        y_amount: int, 
        ...
        try:
            ...
            request_data = {
                ...
                "totalXAmount": str(x_amount),
                "totalYAmount": str(y_amount),
                "maxBinId": strategy_json["maxBinId"],
                "minBinId": strategy_json["minBinId"],
                "strategyType": strategy_json["strategyType"]
            }
            
            logger.info(f"Sending request with data: {json.dumps(request_data, indent=2)}")
            
            result = self.__session.post(
                f"{API_URL}/dlmm/initialize-position-and-add-liquidity-by-strategy", 
            ...
            logger.error(f"Error in initialize_position_and_add_liquidity_by_strategy: {str(e)}")
    ...
    def remove_liqidity(self, position_pub_key: Pubkey, user: Pubkey, bin_ids: List[int], bps: int, should_claim_and_close: bool) -> List[Transaction]:
        ...
        if type(should_claim_and_close) != bool:
        ...
        try:
            ...
            # 打印 API 返回的結果以進行調試
            logger.info(f"API response for remove_liquidity: {result}")
            
            if isinstance(result, list):
                return [self._create_transaction(tx_data) for tx_data in result]
            return [self._create_transaction(result)]
        except requests.exceptions.HTTPError as e:
            raise HTTPError(f"Error removing liquidity: {e}")
        except requests.exceptions.ConnectionError as e:
            raise HTTPError(f"Error connecting to DLMM: {e}")
    
    def _create_transaction(self, tx_data: dict) -> Transaction:
        """Helper method to create a transaction with recent blockhash"""
        try:
            # 如果 API 沒有提供 blockhash，使用 RPC 獲取
            if "recentBlockhash" not in tx_data:
                client = Client(self.rpc)
                recent_blockhash = client.get_latest_blockhash().value.blockhash
                tx_data["recentBlockhash"] = recent_blockhash
                
            if "feePayer" not in tx_data:
                tx_data["feePayer"] = str(tx_data.get("userPublicKey", tx_data.get("user")))
                
            return convert_to_transaction(tx_data)
        except Exception as e:
    ...
    def close_position(
        self,
        position: Position,  # 完整的 Position 對象
        owner: Pubkey  # solders.pubkey.Pubkey 類型
    ) -> Transaction:
        """
        關閉倉位
        """
        try:
            request_data = {
                "position": {
                    "publicKey": str(position.public_key),
                    "positionData": position.position_data.to_json(),
                    "version": position.version
                },
                "owner": str(owner)
            }
            
            logger.info(f"Sending close position request with data: {json.dumps(request_data, indent=2)}")
            
            result = self.__session.post(
                f"{API_URL}/dlmm/close-position",
                data=json.dumps(request_data),
                headers={"Content-Type": "application/json"}
            ).json()
    ...
    def swap_quote(self, amount: int, swap_Y_to_X: bool, allowed_slippage: int, binArrays: List[dict], is_partial_filled: Optional[bool]=False) -> SwapQuote:
        ...
        try:
            ...
            logger.info(f"Swap quote result: {result}") #albert
    ...
    def claim_all_rewards(self, owner: Pubkey, positions: List[Position]) -> List[Transaction]:
        '''
        一次性領取所有倉位的所有獎勵（包括流動性挖礦和交易費用）。

        Args:
            owner (Pubkey): 倉位擁有者的公鑰
            positions (List[Position]): 要領取獎勵的倉位列表
        '''
        if type(owner) != Pubkey:
            raise TypeError("owner must be of type `solders.pubkey.Pubkey`")
        ...
        try:
            ...
            logger.info(f"Sending claim all rewards request with data: {json.dumps(json.loads(data), indent=2)}")
            
            result = self.__session.post(
                f"{API_URL}/dlmm/claim-all-rewards",
                data=data,
                headers={"Content-Type": "application/json"}
            ).json()
    ...
    def claim_reward(
        ...
        try:
            request_data = {
                "positionPubKey": str(position.public_key),
                "userPublicKey": str(owner)
            }
            
            logger.info(f"Sending claim reward request with data: {json.dumps(request_data, indent=2)}")
            
            result = self.__session.post(
                f"{API_URL}/dlmm/claim-reward",
                data=json.dumps(request_data),
                headers={"Content-Type": "application/json"}
            ).json()
